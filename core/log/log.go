// Package log is a logger used by asha core and domains. It standardizes
// logging formats consistent with Asha's needs, allowing for a debug mode, and
// enabling domains to specify their own name so each domain's logs are easy to
// isolate.
package log
...
/*	lg := log.New(os.Stdout, &log.JSONFormatter{})
	if len(pluginName) == 0 {
